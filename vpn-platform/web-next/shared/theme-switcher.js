/* ============================================================================
   AiMA VPN — меню настроек: живое переключение 6 тем + reduced-motion.
   Сохраняет выбор в localStorage (aima-vpn-theme, aima-vpn-rm).
   Использование:
     AimaThemer.init({ defaultTheme: "aurora-glass", lockToPage: false })
   lockToPage=true — экран форсит свою тему, но переключатель всё равно работает.
   ============================================================================ */
(function () {
  const LS_THEME = "aima-vpn-theme";
  const LS_RM = "aima-vpn-rm";
  ...
  function buildPanel(current) {
    const themes = window.AIMA_THEMES || [];
    const rmOn = localStorage.getItem(LS_RM) === "1";

    const backdrop = document.createElement("div");
    backdrop.className = "settings-backdrop";
    backdrop.id = "aima-settings-backdrop";

    const panel = document.createElement("div");
    panel.className = "settings-panel";
    panel.id = "aima-settings-panel";
    ...
    panel.innerHTML = `
      <div class="flex items-center justify-between" style="margin-bottom:var(--sp-4)">
        ...
        <button class="gear" id="aima-settings-close" aria-label="Закрыть">✕</button>
      </div>
      <div class="hint">Тема применяется мгновенно и сохраняется в этом браузере.</div>
      <div class="theme-list" id="aima-theme-list"></div>
      <div class="toggle-row">
        <span>Уменьшить анимацию</span>
        <button class="switch ${rmOn ? "on" : ""}" id="aima-rm-switch" role="switch" aria-checked="${rmOn}" aria-label="Уменьшить анимацию"></button>
      </div>
      <div class="hint" style="margin-top:var(--sp-4)" id="aima-back-link"></div>
    `;
    ...
    const rmSwitch = panel.querySelector("#aima-rm-switch");
    ...
    panel.querySelector("#aima-settings-close").addEventListener("click", close);
  ...
  window.AimaThemer = {
    init(opts) {
      ...
      const gear = document.getElementById("aima-gear");
